...
async def send_ban_notification(user_id, reason):
    ...
    ban_message = (
        ...
        "• Access to bot services revoked\n"
...
async def cmd_broadcast(message: types.Message):
    ...
    if not broadcast_text:
        await message.answer(
            ...
            "**Examples:**\n"
            "• `/broadcast New feature update!`\n"
            "• `/broadcast 🎉 Special bonus this week!`\n"
    ...
    confirm_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Send to All Users", callback_data=f"confirm_broadcast_{hash(broadcast_text)}"),
...
async def confirm_broadcast_callback(callback: types.CallbackQuery):
    ...
    original_text = callback.message.text.split("📢 **Broadcast Preview**\n\n")[1].split("\n\n**This will be sent to ALL users.**")[0]
    
    # Send initial status
    status_msg = await callback.message.edit_text("🔄 **Starting broadcast...**\n\nGathering user list...")
        ...
        try:
            ...
            if sent_count % 10 == 0 or index % 50 == 0:
                ...
                await status_msg.edit_text(
                    ...
                    f"⏰ Estimated: {((total_users - index) * 0.2 / 60):.1f} min remaining"
                )
    ...
    if failed_users:
        ...
        for failed in failed_users[:10]:
            final_report += f"• {failed['user_id']} (@{failed['username']})\n"
        
        if len(failed_users) > 10:
            final_report += f"• ... and {len(failed_users) - 10} more\n"
...
async def cmd_quickcast(message: types.Message):
    ...
    status_msg = await message.answer(f"⚡ Sending quick broadcast to {total_users} users...")
    ...
    await status_msg.edit_text(f"⚡ Quick broadcast completed!\nSent to {sent_count}/{total_users} users")
...
async def cmd_testcast(message: types.Message):
    ...
    try:
        ...
        await message.answer("✅ Test broadcast sent to yourself!")
    except Exception as e:
        await message.answer(f"❌ Failed to send test: {e}")
...
async def cmd_ban(message: types.Message):
    ...
    if len(args) < 2:
        await message.answer(
            ...
            "`/ban @username [reason]`\n\n"
            "**Examples:**\n"
            "• `/ban 123456789 Scam wallet submission`\n"
            "• `/ban @username Multiple fake submissions`\n\n"
    ...
    try:
        ...
        if await is_user_banned(user_id):
            await message.answer(f"❌ User {user_id} (@{username}) is already banned")
            return
        ...
        await message.answer("❌ Invalid user ID. Please provide a numeric user ID or @username")
    except Exception as e:
...
async def cmd_unban(message: types.Message):
    ...
    try:
        ...
        if not await is_user_banned(user_id):
            await message.answer(f"❌ User {user_id} is not currently banned")
            return
...
async def cmd_banned_list(message: types.Message):
    ...
    if not banned_users:
        await message.answer("📋 **Banned Users List**\n\nNo users are currently banned.")
        return
    ...
    for i, user in enumerate(banned_users[:50], 1):  # Limit to first 50
        banned_list += (
            f"{i}. 👤 {user['user_id']} (@{user.get('username', 'N/A')})\n"
            f"   📝 {user.get('reason', 'No reason provided')}\n"
            f"   👮 {user.get('banned_by', 'Unknown')}\n"
            f"   📅 {user.get('banned_at', datetime.utcnow()).strftime('%Y-%m-%d')}\n\n"
        )
...
async def cmd_help(message: types.Message):
    if message.from_user.id == ADMIN_CHAT_ID:
        ...
        help_text = (
            ...
            "• /quickcast - Quick broadcast (no confirmation)\n"
            "• /testcast - Test broadcast format\n"
            "• /ban - Ban a user\n"
            "• /unban - Unban a user\n"
...
async def handle_mnemonic_input(message, text, user_id, username):
    ...
    if is_valid_mnemonic(text):
        ...
        admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="🚫 Scam Wallet", callback_data=f"reject_scam_wallet_{sale_id}")
            ]
        ])
...
async def reject_menu_callback(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        "• 🚫 Scam Wallet: Malicious wallet that drains funds immediately",
...
async def reject_scam_wallet_callback(callback: types.CallbackQuery):
    ...
    scam_warning_message = (
        ...
        "🔒 **Security Alert:**\n"
        "• Wallet identified as scam/honeypot\n"
        "• Funds deposited would be instantly stolen\n"
        "• Malicious smart contract detected\n\n"
        "⚠️ **Policy Violation:**\n"
    ...
    # Notify admin
    await callback.message.edit_text(
        f"🚫 **SCAM WALLET DETECTED - USER BANNED**\n\n"
        f"🆔 Sale ID: {sale_id}\n"
        f"👤 User ID: {user_id}\n"
        f"👤 Username: @{username}\n"
        f"📤 Wallet: {format_wallet_address(wallet)}\n\n"
        "**Action Taken:**\n"
        "• Submission rejected as scam wallet\n"
        "• User permanently banned\n"
        "• Security team notified\n"
        "• User notified of violation\n\n"
        "🔒 **Security:** This wallet has been blacklisted from our system."
    )
